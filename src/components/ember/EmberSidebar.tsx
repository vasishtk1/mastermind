...
const items = [
  { to: "/", icon: Cpu, label: "Researcher IDE" },
  { to: "/sentinel", icon: Activity, label: "Edge Sentinel" },
];

export const EmberSidebar = () => {
  return (
    <aside className="w-16 shrink-0 bg-surface border-r border-border/60 flex flex-col items-center py-5 gap-2 z-10">
      <div className="w-9 h-9 rounded-lg bg-gradient-to-br from-primary to-secondary grid place-items-center mb-4 glow-teal">
          ...
          <Tooltip key={to}>
            <TooltipTrigger asChild>
              <NavLink
                ...
                className={({ isActive }) =>
                  `w-10 h-10 rounded-lg grid place-items-center transition-all border ${
                    isActive
